...
class PaymentRepository extends \Doctrine\ORM\EntityRepository
{
	...
	public function findNbCommandClient($client,$dates){
		$qb = $this->createQueryBuilder('payment');
		return $qb->select('COUNT(DISTINCT payment.uuid)')->where('payment.vadsEffectiveCreationDate > ?1 ')->andWhere("payment.vadsCustId = ?2")->setParameter(2,$client)->andWhere('payment.vadsEffectiveCreationDate < ?3 ')->setParameters(array(1=>$dates['dateBefore'],2=>$client,3=>$dates['dateAfter']))->getQuery()->getSingleScalarResult();
	}
	...
	public function findTtAmountClient($client,$dates){
		$qb = $this->createQueryBuilder('payment');
		return $qb->select('SUM(payment.vadsEffectiveAmount)')->where('payment.vadsEffectiveCreationDate > ?1 ')->andWhere("payment.vadsCustId = ?2")->setParameter(2,$client)->andWhere('payment.vadsEffectiveCreationDate < ?3 ')->andWhere("payment.vadsTransStatus != 'REFUSED'")->setParameters(array(1=>$dates['dateBefore'],2=>$client,3=>$dates['dateAfter']))->getQuery()->getSingleScalarResult() + 0;
	}

	public function findNbCommand(DateTime $dateBefore,DateTime $dateAfter){
		$qb = $this->createQueryBuilder('payment');
		return $qb->where('payment.vadsEffectiveCreationDate > ?1 ')->setParameter(1,$dateBefore)->andWhere('payment.vadsEffectiveCreationDate <= ?2 ')->setParameter(2,$dateAfter)->select('COUNT(payment)')->getQuery()->getSingleScalarResult();
	}
	...
	public function findNbRefused(DateTime $dateBefore, DateTime $dateAfter){
		$qb = $this->createQueryBuilder('payment');
		return $qb->where('payment.vadsEffectiveCreationDate > ?1 ')->setParameter(1,$dateBefore)->andWhere('payment.vadsEffectiveCreationDate <= ?2 ')->setParameter(2,$dateAfter)->select('COUNT(DISTINCT payment.uuid)')->andWhere("payment.vadsTransStatus = 'REFUSED'")->getQuery()->getSingleScalarResult();
	}

	public function findTtAmount(DateTime $dateBefore,DateTime $dateAfter){
		$qb = $this->createQueryBuilder('payment');
		return $qb->select('SUM(payment.vadsEffectiveAmount)')->where('payment.vadsEffectiveCreationDate > ?1 ')->setParameter(1,$dateBefore)->andWhere('payment.vadsEffectiveCreationDate <= ?2 ')->setParameter(2,$dateAfter)->andWhere("payment.vadsTransStatus != 'REFUSED'")->andWhere("payment.vadsPaymentType != 'RETRY'")->getQuery()->getSingleScalarResult() + 0 - $qb->select('SUM(payment.vadsEffectiveAmount)')->where('payment.vadsEffectiveCreationDate > ?1 ')->setParameter(1,$dateBefore)->andWhere('payment.vadsEffectiveCreationDate <= ?2 ')->setParameter(2,$dateAfter)->andWhere("payment.vadsTransStatus != 'REFUSED'")->andWhere("payment.vadsPaymentType = 'RETRY'")->getQuery()->getSingleScalarResult() ;
	}
